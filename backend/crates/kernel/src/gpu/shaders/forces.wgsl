...
fn update_boundary_pressures(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Search all fluid particles near this boundary particle
    // For simplicity, iterate over all fluid particles (same as CPU version)
    for (var f = 0u; f < params.n_particles; f = f + 1u) {
        let dx = bx - pos_x[f];
        let dy = by - pos_y[f];
        let dz = bz - pos_z[f];
...
fn compute_forces(@builtin(global_invocation_id) gid: vec3<u32>) {
        ...
        for (var dy_off = -1; dy_off <= 1; dy_off = dy_off + 1) {
            ...
            for (var dx_off = -1; dx_off <= 1; dx_off = dx_off + 1) {
                ...
                for (var s = start; s < start + count; s = s + 1u) {
                    ...
                    let p_factor = -m_i * mass[j] * (pi_over_rho2_i + pj_over_rho2_j);
    ...
    // Boundary particle contributions (pressure forces)
    ...
    // Boundary repulsive forces (safety net)
    let r0 = 0.5 * h;
    ...
    for (var b = 0u; b < params.n_boundary; b = b + 1u) {
        ...
        if r < r0 && r > 1.0e-12 {
            ...
            let inv_r = 1.0 / r;
            // Repulsive forces applied as acceleration (not divided by mass again)
